...
class Smarty_Internal_Runtime_Inheritance
{
     ...
     */
    public function processBlock(Smarty_Internal_Template $tpl, $type, $name, $block, $callStack = [])
    {
        if (!isset($this->blockParameter[$name])) {
            $this->blockParameter[$name] = [];
        }
        if ($this->state == 1) {
