    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter)
    {
        ...
        if (isset($_attr['assign'])) {
            ...
            if (!empty($parameter['modifierlist'])) {
                $output = $compiler->compileTag('private_modifier', [], ['modifierlist'           => $parameter['modifierlist'],
                                                                                   'value'        => $output, ]);
            }
            if (!$_attr['nofilter']) {
                // default modifier
                if (!empty($compiler->smarty->default_modifiers)) {
                    ...
                    $output = $compiler->compileTag('private_modifier', [], ['modifierlist'           => $compiler->default_modifier_list,
                                                                                       'value'        => $output, ]);
                }
                // autoescape html
                ...
                foreach ($compiler->variable_filters as $filter) {
                    if (count($filter) == 1 &&
                        ...
                        $output = $compiler->compileTag('private_modifier', [], ['modifierlist'           => [$filter],
                                                                                           'value'        => $output, ]);
                    }
                }
            }
