...
function Glovar($key = '', $val = '', $region = 'User')
{
    global $php300global;
    if (empty($key)) {
        return (is_array($php300global[$region])) ? ($php300global[$$region]) : ([]);
    }
    if (empty($val)) {
...
{
    $Data = ['Msg'    =>$Msg, 'Status' =>'1', 'Seconds'=>$Seconds, 'Url'    =>$Url];
    ShowPage($Data);
}
...
{
    $Data = ['Msg'    =>$Msg, 'Status' =>'0', 'Seconds'=>$Seconds, 'Url'    =>$Url];
    ShowPage($Data);
}
...
function Yun($Update = false)
{
    ...
    if (file_exists($fileName)) {
        ...
        if ($Config['Type'] == '1') {
            ...
            if (!empty($cacheClass)) {
                $cacheClass = json_decode($cacheClass, true);
                $Code = '';
