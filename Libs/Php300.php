...
class Php300
{
     ...
     * 普通get执行.
     */
    public function Directly($UrlConfig)
    {
     ...
     * @param URL配置    $Config
     */
    public function Arrgd($QueryUrl, $Config)
    {
        if ($QueryUrl != '') {
    ...
    public function setConstant()
    {
        ...
        $Path = explode('/', $Path);
        $Path = (!empty($Path[1])) ? ((strpos($Path[1], '.php') === false) ? ('/'.$Path[1].'/') : ('/')) : ('/');
        $DefineArr = [
            '__APP__'      => $Path,
